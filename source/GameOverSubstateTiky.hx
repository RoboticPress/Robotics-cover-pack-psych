...
class GameOverSubstateTiky extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		sign.frames = Paths.getSparrowAtlas('expurgation/signDeath','shared');
		...
		FlxG.sound.play(Paths.sound('expurgation/BF_Deathsound','shared'));
		FlxG.sound.play(Paths.sound('expurgation/Micdrop','shared'));
	...
	override function update(elapsed:Float)
	{
		...
		{
			FlxG.sound.playMusic(Paths.music('expurgation/gameOverTiky','shared'));
			sign.x += 131;
			sign.x += 131;
	...
	function endBullshit():Void
	{
		if (!isEnding)
		{
			...
			//sign.screenCenter();
			FlxG.sound.music.stop();
			FlxG.sound.play(Paths.music('expurgation/gameOverEndTiky','shared'));
